...
class FileThemeNodeContentRenderer extends Component {
  render() {
    ...
    const nodeContent = (
      <div style={{ height: '100%' }} className={`${+node.id === +selectedNodeId ? styles.selectedNode: ''}`} {...otherProps}>
        {toggleChildrenVisibility &&
          node.children &&
          node.children.length > 0 && (
            <button
              ...
              style={{
                left: (lowerSiblingCounts.length - 0.4) * scaffoldBlockPxWidth,
              }}
              onClick={() =>
                toggleChildrenVisibility({
                  node,
